...
I used about 50,000 data points from the larger set to run this model. I am waiting to run it on the whole set until I work through the remaining issues, just to cut down on run time. In this section, I need to preprocess more neatly and work on adding a lemmatizer, in addition to a stemmer, as the stemmer here isn't as effective as I had hoped it would be, hence the manual removals of some words.

Lastly, I used multiple methods for stop word removal, which is probably not the best practice. However, these methods seemed to work best in tandem with one another.
...
# Load and process data part one
#pdata_set_p1<- read.csv("~/Desktop/covid_disinfo_data.csv", head(T))
...
Here, I chose a k value of 15. This appeared to garner the most accurate results, when compared to k=10 and k=20, however, I still need to further experiment with a wider range of potential k values to improve this model's accuracy.
...
This code sorts and orders the topics by how often they emerge in the documents as an entire collection. Most common is the topic that corresponds with the corpus " case updat death number toll." This topic is likely "Case/death counts," or something similar.
